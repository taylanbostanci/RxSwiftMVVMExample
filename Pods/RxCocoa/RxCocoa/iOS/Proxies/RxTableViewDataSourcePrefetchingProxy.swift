...
private final class TableViewPrefetchDataSourceNotSet
    ...
    func tableView(_ taskTV: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {}
...
open class RxTableViewDataSourcePrefetchingProxy
    ...
    /// Typed parent object.
    public weak private(set) var taskTV: UITableView?

    /// - parameter tableView: Parent object for delegate proxy.
    public init(taskTV: ParentObject) {
        self.taskTV = taskTV
        super.init(parentObject: taskTV, delegateProxy: RxTableViewDataSourcePrefetchingProxy.self)
    }

    // Register known implementations
    public static func registerKnownImplementations() {
        self.register { RxTableViewDataSourcePrefetchingProxy(taskTV: $0) }
    }
    ...
    public func tableView(_ taskTV: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        if let subject = _prefetchRowsPublishSubject {
            subject.on(.next(indexPaths))
        }

        (_requiredMethodsPrefetchDataSource ?? tableViewPrefetchDataSourceNotSet).tableView(taskTV, prefetchRowsAt: indexPaths)
    }
